...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  // Based on https://www.trafitec.dk/sites/default/files/publications/braking%20distance%20-%20friction%20and%20driver%20behaviour.pdf
  const double deceleration = 3.4;
  double stopping_distance = (velocity_mag * velocity_mag) / (2 * deceleration);
  
  auto look_ahead_distance = stopping_distance;
